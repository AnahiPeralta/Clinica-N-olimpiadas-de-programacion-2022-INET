...
<head>
    ...
    <link rel="stylesheet" href="./css/medic-style.css">
...
$consulta = "SELECT * FROM medicos $where";
...
<?php
    include("Config/Conexion.php");
        $sql = "SELECT * FROM medicos WHERE area ='Pediatria'";
        $query = mysqli_query($conexion, $sql);

        while ($fila = mysqli_fetch_array($query)) {
        ?>
    <h1><?php echo $fila['area']?></h1>
    <div class="card">
        ...
        <div class="info">
            ...
            <div class="boton2">
            <a name="" id="" class="btn btn-primary" href="CRUD/ver_mas2.php?id=<?php echo $fila['id'] ?>" role="button"> ver mas </a>
            </div>
        </div>
    </div>
    <?php
        ...
        <center><i class='bx bxs-message-square-add'>
                    <a href="Forms/AgregarCliente.php" class="btn btn-add">Añadir Médico</a></i></center>
         </div>
</div>
...
<?php
    include("Config/Conexion.php");
        $sql = "SELECT * FROM medicos WHERE area ='Urgencias'";
        $query = mysqli_query($conexion, $sql);
    ...
    <div class="card">
        ...
        <div class="info">
            ...
            <p>DNI <?php echo $fila['DNI']?></p>
            <div class="boton2">
            <a name="" id="" class="btn btn-primary" href="CRUD/ver_mas2.php?id=<?php echo $fila['id'] ?>" role="button"> Ver más </a>
            </div>
        </div>
    </div>
    <?php
...
<?php
    include("Config/Conexion.php");
        $sql = "SELECT * FROM medicos WHERE area ='Traumatologia'";
        $query = mysqli_query($conexion, $sql);
...
<?php
    include("Config/Conexion.php");
        $sql = "SELECT * FROM medicos WHERE area ='Obstetricia'";
        $query = mysqli_query($conexion, $sql);
